...
export default function Login() {
  ...
  const [req, setReq] = useState({
    full_name: '', email: '', phone: '', program: 'applied_ai',
  ...
  return (
    <div style={s.wrap}>
      <div style={s.bgGlow1} /><div style={s.bgGlow2} /><div style={s.bgGlow3} />
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {[['en', '🇺🇸 English'], ['ar', '🇸🇦 العربية'], ['fa', '🇮🇷 فارسی']].map(([code, label]) => (
                <button key={code} onClick={() => { i18n.changeLanguage(code); document.documentElement.dir = code !== 'en' ? 'rtl' : 'ltr' }}
                  style={{ padding: '5px 12px', borderRadius: 20, border: `1px solid ${i18n.language === code ? '#F97316' : 'rgba(255,255,255,0.1)'}`, background: i18n.language === code ? 'rgba(249,115,22,0.15)' : 'transparent', color: i18n.language === code ? '#F97316' : '#6B7A96', fontSize: '0.74rem', cursor: 'pointer', fontFamily: 'inherit' }}>
                  {label}
                </button>
              ))}
            </div>
        ...
        <div style={s.formWrap} className="fade-up">
          ...
          {tab === 'login' && (
            <form onSubmit={handleSignIn}>
              ...
              <p style={s.formDesc}>Sign in with the credentials provided by your instructor.</p>
              ...
              <div style={s.helpBox}>
                ...
                <button type="button" onClick={() => setTab('request')} style={{ color: '#F97316', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 'inherit', fontWeight: 700 }}>Request Access →</button>
              </div>
            </form>
          )}
              ...
              <form onSubmit={handleRequestAccess}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', margin: '1.25rem 0' }}>
                  <div className="form-group">
                    ...
                    <input className="input" required value={req.full_name} onChange={e => setReq(r => ({ ...r, full_name: e.target.value }))} placeholder="First and last name" />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Email Address *</label>
                    <input type="email" className="input" required value={req.email} onChange={e => setReq(r => ({ ...r, email: e.target.value }))} placeholder="your@email.com" />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Phone (optional)</label>
                    <input type="tel" className="input" value={req.phone} onChange={e => setReq(r => ({ ...r, phone: e.target.value }))} placeholder="(720) 000-0000" />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Program You'd Like to Enroll In *</label>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 4 }}>
                      {PROGRAMS.map(p => (
                        <button key={p.id} type="button" onClick={() => setReq(r => ({ ...r, program: p.id }))}
                          style={{ textAlign: 'left', padding: '10px 14px', borderRadius: 8, border: `1px solid ${req.program === p.id ? p.color : 'rgba(255,255,255,0.1)'}`, background: req.program === p.id ? `${p.color}15` : 'rgba(255,255,255,0.03)', color: req.program === p.id ? p.color : '#B0BACA', fontSize: '0.8rem', cursor: 'pointer', fontFamily: 'inherit', fontWeight: req.program === p.id ? 700 : 400, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={reqLoading} style={{ width: '100%', height: 48, fontSize: '0.92rem', borderRadius: 10 }}>
                  {reqLoading ? <><div className="loader-sm" style={{ marginRight: 8 }} /> Submitting…</> : '✋ Submit Request'}
                </button>
...
const s = {
  wrap: { minHeight: '100vh', display: 'flex', background: '#0C0F14', position: 'relative', overflow: 'hidden' },
  bgGlow1: { position: 'absolute', top: -200, left: -200, width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(249,115,22,0.09) 0%, transparent 65%)', pointerEvents: 'none' },
  ...
  bgGlow3: { position: 'absolute', top: 200, right: -100, width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(34,197,94,0.05) 0%, transparent 65%)', pointerEvents: 'none' },
  ...
  helpBox: { marginTop: '1.25rem', textAlign: 'center', fontSize: '0.78rem', color: '#6B7A96', lineHeight: 1.8 },
